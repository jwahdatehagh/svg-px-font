...
contract TypeWriter {
    ...
    /// @dev Write a text as an SVG pixel font inheriting text color and letter-spaced with 1px.
    function write(string memory text) public view returns (string memory) {
        return write(text, "currentColor", 1);
    }

    /// @dev Write a text as an SVG pixel font with 1px space between letters.
    function write(string memory text, string memory color) public view returns (string memory) {
        return write(text, color, 1);
    }

    /// @dev Write a given text as an SVG pixel font in given `color` and letter `spacing`.
    function write(
        string memory text,
        string memory color,
        uint256 spacing
        ...
        string memory svg = string(abi.encodePacked(
            "<g fill-rule='evenodd' clip-rule='evenodd' fill='",
            color,
            "' aria-label='",
            text,
            "'>",
    ...
    function initializeFont() private {
        // Special
        LETTERS[" "] = "<rect x='1' y='5' width='1' height='1' fill='none'/>";
